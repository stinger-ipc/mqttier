...
pub struct MetricsSnapshot {
    ...
    pub max_publish_latency_ms: u64,
    pub min_publish_latency_ms: u64,
...
pub struct Metrics {
    ...
    // Performance metrics
    pending_publish_count: AtomicU32,
    total_publish_latency_ms: AtomicU64, // For calculating average
    publish_latency_samples: AtomicU64,  // Number of latency samples
    max_publish_latency_ms: AtomicU64,
    min_publish_latency_ms: AtomicU64,
...
impl Metrics {
    ...
    pub fn new() -> Self {
        Self {
            min_publish_latency_ms: AtomicU64::new(u64::MAX),
            ..Default::default()
        }
    }
    ...
    pub fn record_successful_connection(&self) {
        ...
        let now = SystemTime::now()
            ...
            .unwrap_or(Duration::from_secs(0))
            .as_millis() as u64;
        self.connection_start_timestamp.store(now, Ordering::Relaxed);
    }
    ...
    pub fn record_disconnection(&self) {
        if self.is_connected.load(Ordering::Relaxed) {
            ...
            if start > 0 {
                ...
                self.last_connection_duration_ms.store(duration, Ordering::Relaxed);
            }
            self.is_connected.store(false, Ordering::Relaxed);
        }
    }
    ...
    pub fn record_message_published(&self, qos: u8, payload_size: usize) {
        ...
        self.total_bytes_sent.fetch_add(payload_size as u64, Ordering::Relaxed);
    }
    ...
    pub fn increment_pending_publishes(&self) {
        self.pending_publish_count.fetch_add(1, Ordering::Relaxed);
    }
    
    pub fn decrement_pending_publishes(&self) {
        self.pending_publish_count.fetch_sub(1, Ordering::Relaxed);
    }
    
    pub fn record_publish_latency(&self, latency_ms: u64) {
        self.total_publish_latency_ms.fetch_add(latency_ms, Ordering::Relaxed);
        self.publish_latency_samples.fetch_add(1, Ordering::Relaxed);
        
        // Update max
        self.max_publish_latency_ms.fetch_max(latency_ms, Ordering::Relaxed);
        
        // Update min
        let mut current_min = self.min_publish_latency_ms.load(Ordering::Relaxed);
        while latency_ms < current_min {
            match self.min_publish_latency_ms.compare_exchange_weak(
                current_min,
                latency_ms,
                Ordering::Relaxed,
                Ordering::Relaxed,
    ...
    pub fn record_message_received(&self, qos: u8, payload_size: usize) {
        ...
        self.total_bytes_received.fetch_add(payload_size as u64, Ordering::Relaxed);
    }
    ...
    pub fn snapshot(&self) -> MetricsSnapshot {
        ...
        let avg_latency = if latency_samples > 0 {
            self.total_publish_latency_ms.load(Ordering::Relaxed) / latency_samples
        } else {
            0
        };
        
        let min_latency = self.min_publish_latency_ms.load(Ordering::Relaxed);
        ...
        MetricsSnapshot {
            ...
            connection_start_timestamp: if connection_start > 0 { Some(connection_start) } else { None },
            last_connection_duration_ms: if last_conn_duration > 0 { Some(last_conn_duration) } else { None },
            ...
            pending_publish_count: self.pending_publish_count.load(Ordering::Relaxed),
            avg_publish_latency_ms: avg_latency,
            max_publish_latency_ms: self.max_publish_latency_ms.load(Ordering::Relaxed),
            min_publish_latency_ms: min_latency,
    ...
    pub fn reset(&self) {
        ...
        self.pending_publish_count.store(0, Ordering::Relaxed);
        self.total_publish_latency_ms.store(0, Ordering::Relaxed);
        self.publish_latency_samples.store(0, Ordering::Relaxed);
        self.max_publish_latency_ms.store(0, Ordering::Relaxed);
        self.min_publish_latency_ms.store(u64::MAX, Ordering::Relaxed);
...
mod tests {
    ...
    fn test_latency_metrics() {
        ...
        let snapshot = metrics.snapshot();
        assert_eq!(snapshot.avg_publish_latency_ms, 116); // (100 + 200 + 50) / 3
        assert_eq!(snapshot.max_publish_latency_ms, 200);
        assert_eq!(snapshot.min_publish_latency_ms, 50);
    }
